...
def write_html_report(
    ...
    with open(html_path, "w", encoding="utf-8") as fh:
        ...
        fh.write(
            ...
            "}\n"
            "function _molLooksValid(mol){\n"
            "  if(!mol){return false;}\n"
            "  if(typeof mol.is_valid==='function'){\n"
            "    try{return !!mol.is_valid();}catch(_molValidErr){return false;}\n"
            "  }\n"
            "  if(typeof mol.get_smiles==='function'){\n"
            "    try{return String(mol.get_smiles()||'').trim().length>0;}catch(_molSmilesErr){return false;}\n"
            "  }\n"
            "  if(typeof mol.get_smarts==='function'){\n"
            "    try{return String(mol.get_smarts()||'').trim().length>0;}catch(_molSmartsErr){return false;}\n"
            "  }\n"
            "  return false;\n"
            "}\n"
            "function _qmolLooksValid(qmol){\n"
            "  if(!qmol){return false;}\n"
            "  if(typeof qmol.is_valid==='function'){\n"
            "    try{return !!qmol.is_valid();}catch(_qmolValidErr){return false;}\n"
            "  }\n"
            "  if(typeof qmol.get_smarts==='function'){\n"
            "    try{return String(qmol.get_smarts()||'').trim().length>0;}catch(_qmolSmartsErr){return false;}\n"
            "  }\n"
            "  return false;\n"
            "}\n"
            ...
            "      if(_molLooksValid(srcMol)){\n"
            "        try{\n"
            "          try{previewUrl=_svgToDataUrl(srcMol.get_svg());}catch(_svgErr){}\n"
            "          queryMol=RDKit.get_qmol(inputText);\n"
            "          if(!_qmolLooksValid(queryMol)){throw new Error('Could not build a valid substructure exclusion from the pasted SMILES.');}\n"
            "        }finally{srcMol.delete();}\n"
            "      } else {\n"
            "        if(srcMol){srcMol.delete();}\n"
            "        queryMol=RDKit.get_qmol(inputText);\n"
            "        if(!_qmolLooksValid(queryMol)){\n"
            ...
            "  try {\n"
            "    if(!_molLooksValid(mol)){throw new Error('Exact match requires a valid SMILES string.');}\n"
            ...
            "    }\n"
            "    try{\n"
            "      if(typeof rawMatches.length==='number'){return Array.prototype.slice.call(rawMatches);}\n"
            "    }catch(_lenErr){}\n"
            "    return [];\n"
            "  }\n"
            ...
            "      var hitIndexes=_toMatchIndexes(library.get_matches(queryMol));\n"
            ...
            "          try{\n"
            "            queryMol=RDKit.get_qmol(inputText);\n"
            "            if(!_qmolLooksValid(queryMol)){throw new Error('Could not build a valid substructure query from the pasted SMILES.');}\n"
            "          }finally{srcMol.delete();}\n"
            "        } else {\n"
            "          if(srcMol){srcMol.delete();}\n"
            "          queryMol=RDKit.get_qmol(inputText);\n"
