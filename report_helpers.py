...
def build_idea_cards(
    ...
    for _, row in df.iterrows():
        ...
        if include_links and card_type == "Central":
            blocks.append(
                f"<div class='smalltxt' style='margin-bottom:6px;'><a href='javascript:void(0)' onclick=\"openDeepDive('{sname_js}')\">Go to deep dive</a></div>"
            )
        ...
        blocks.append(
            f"<div class='smalltxt'>"
            f"<b>Members:</b> <span class='central-members-count' data-raw-members='{n_members}' data-unique-members='{unique_members}'>{n_members}</span> | "
        ...
        blocks.append(f"<div class='smalltxt'><b>Pose IDs to inspect:</b> <span class='mono'>{row.get('representative_ids', '')}</span></div>")
        blocks.append("</div>")
    return "".join(blocks)
...
def build_deep_dive_html(
    ...
    parts.append("<ul>")
    parts.append(f"<li><b>Members:</b> <span class='dd-members-count' data-raw-members='{int(row['n_members'])}'>{int(row['n_members'])}</span></li>")
...
def write_html_report(
    ...
    structure_search_entries = build_structure_search_entries(
        mol_df,
    ...
    kpi = {
        ...
        "Scaffolds": int(scaf_df["scaffold_id"].nunique()),
        "Central Ideas": int(len(central_df)),
    }
    ...
    with open(html_path, "w", encoding="utf-8") as fh:
        ...
        fh.write("<h1>R-group Docking Insight Report</h1>")
        fh.write("<p class='small'>Automated pattern discovery and medicinal chemistry prioritization from docking SDF plus interaction-count CSV.</p>")
        ...
        fh.write(
            ...
            "Two download options exist: exporting all molecules from a single scaffold or exporting selected individual molecules from deep dive sections.</li>"
        )
        fh.write(
        ...
        fh.write(
            ...
            "Users can overlay multiple molecules (2-5) in the 3D viewer to compare docking poses and interactions, with molecules color-coded and aligned by scaffold.</li>"
        )
        fh.write(
            "<li><b>Exporting scaffold series:</b> "
            "Users can select entire scaffold series for export by checking scaffold select, enabling all members inclusion, and exporting poses in SDF format.</li>"
        )
        fh.write("</ul>")
        fh.write(
            "<p class='smalltxt'>"
        ...
        if hbond_residue_options:
            ...
            fh.write("<p class='smalltxt'>Select one or more residues to keep only central scaffolds whose members make all selected hydrogen bonds, based on donor/acceptor interaction columns in the interaction CSV.</p>")
        ...
        fh.write("<p class='smalltxt'>Paste a SMILES or SMARTS string to filter central scaffold cards and deep-dive sections by substructure or exact full-molecule match.</p>")
        ...
        fh.write("</div>")
        fh.write("<div class='structure-search-status' id='structure-search-status' data-tone='ready'>Central ideas load first. RDKit search loads on first use.</div>")
        ...
        fh.write("<div class='structure-search-body'>")
        fh.write("<p class='smalltxt'>Matches are removed globally from member counts, scaffold downloads, and visible deep-dive tiles. Pasted SMILES are treated as substructure queries by default.</p>")
        ...
        fh.write("</div>")
        fh.write("<div class='structure-search-status' id='exclude-motif-status' data-tone='ready'>No motif exclusion is active.</div>")
        fh.write("<div id='exclude-motif-active-list' style='display:flex;flex-direction:column;gap:8px;'></div>")
        fh.write("<div style='border:1px solid #d6dee8;border-radius:10px;padding:10px;background:#f8fbff;margin-top:8px;'>")
        fh.write("<div class='smalltxt' style='margin-bottom:6px;'><b>SMILES to SMARTS Converter</b></div>")
        fh.write("<textarea id='smiles-to-smarts-input' placeholder='Paste SMILES to convert (e.g. CC(=O)Nc1ccccc1)'></textarea>")
        fh.write("<div class='structure-search-buttons'><button type='button' class='primary' id='smiles-to-smarts-run'>Convert</button></div>")
        fh.write("<textarea id='smiles-to-smarts-output' placeholder='Generated SMARTS appears here' readonly></textarea>")
        fh.write("<div class='structure-search-status' id='smiles-to-smarts-status' data-tone='ready'>Enter a SMILES string, then click Convert.</div>")
        fh.write("</div>")
        fh.write("</div></details></section>")
        ...
        # Keep initial report payload lean: prebuild deep dives only for the top report scaffolds.
        deep_dive_source_df = central_df.head(max_scaffolds_in_report)
        ...
        deep_dive_payload = {}
        for _, row in deep_dive_source_df.iterrows():
            sid = row["scaffold_id"]
            subset = mol_df[mol_df["scaffold_id"] == sid].sort_values("priority_rank").head(top_per_scaffold)
            deep_dive_payload[str(row.get("scaffold_name", "") or "")] = build_deep_dive_html(
                row,
                subset,
                outdir,
                scaffold_export_data=scaffold_export_data,
        ...
        # (not JSON string parsing) — dramatically faster initial load; images are still embedded.
        fh.write("<div id='dd-templates-host' hidden>")
        for _dd_name, _dd_html in deep_dive_payload.items():
            # template id uses scaffold name directly (SCF-NNN format, valid HTML id)
            _dd_tpl_id = "dd-tpl-" + _dd_name
            fh.write(f"<template id='{html.escape(_dd_tpl_id, quote=True)}'>{_dd_html}</template>")
        fh.write("</div>")
        ...
        structure_search_json = json.dumps(structure_search_entries, ensure_ascii=True)
        ...
        fh.write(
            ...
            f"const _STRUCTURE_SEARCH_ENTRIES={structure_search_json};\n"
            "const _STRUCTURE_MATCH_STATE={active:false,mode:'substructure',matchedScaffolds:null,matchedMolIds:null,lastQuery:''};\n"
            ...
            "  status.dataset.tone=tone||'ready';\n"
            "}\n"
            "function _setSmilesToSmartsStatus(message,tone){\n"
            "  var status=document.getElementById('smiles-to-smarts-status');\n"
            "  if(!status){return;}\n"
            "  status.textContent=message;\n"
            "  status.dataset.tone=tone||'ready';\n"
            "}\n"
            ...
            "}\n"
            "function _ensureStructureLibrary(){\n"
            "  if(!_structureLibraryPromise){\n"
            "    _structureLibraryPromise=_ensureRDKit().then(function(RDKit){\n"
            "      var library=new RDKit.SubstructLibrary();\n"
            "      _STRUCTURE_SEARCH_ENTRIES.forEach(function(entry){library.add_trusted_smiles(entry.smiles);});\n"
            "      return {RDKit:RDKit,library:library};\n"
            "    });\n"
            "  }\n"
            "  return _structureLibraryPromise;\n"
            "}\n"
            ...
            "  _ensureStructureLibrary().then(function(){\n"
            ...
            "  return text.split(/\\r?\\n/)[0].trim();\n"
            "}\n"
            "function _getSmilesToSmartsInputText(){\n"
            "  var input=document.getElementById('smiles-to-smarts-input');\n"
            "  var text=String((input&&input.value)||'').trim();\n"
            "  if(!text){return '';}\n"
            "  return text.split(/\\r?\\n/)[0].trim();\n"
            "}\n"
            ...
            "      if(stats.keptCount===0&&stats.totalCount>0){item.scaffoldsRemoved+=1;}\n"
            "    });\n"
            ...
            "  var wrap=document.getElementById('central-filter-impact');\n"
            ...
            "    var currentCounts=_countScaffoldMembers(scaf,{applyPropFilters:false});\n"
            "    var excludeRemoved=Math.max(0,baseCount-currentCounts.raw);\n"
            "    var parts=[];\n"
            "    var titles=[];\n"
            "    if(baseCount>0&&excludeRemoved>0){\n"
            "      var excludePct=Math.round((excludeRemoved*100)/baseCount);\n"
            "      parts.push('motif ↓ '+excludePct+'%');\n"
            ...
            "      parts.push('prop ↓ '+stats.removedPct+'%');\n"
            ...
            "    var bundle=await _ensureStructureLibrary();\n"
            "    var queryMol=null;\n"
            "    var previewUrl='';\n"
            "    try {\n"
            "      var srcMol=bundle.RDKit.get_mol(inputText);\n"
            ...
            "          queryMol=bundle.RDKit.get_qmol(querySmarts);\n"
            ...
            "        queryMol=bundle.RDKit.get_qmol(inputText);\n"
            ...
            "      }\n"
            "      var hitIndexes=JSON.parse(bundle.library.get_matches(queryMol)||'[]');\n"
            "      var matchedEntries=hitIndexes.map(function(idx){return _STRUCTURE_SEARCH_ENTRIES[idx];}).filter(Boolean);\n"
            ...
            "        _setExcludeMotifStatus('No molecules matched the exclusion motif.','working');\n"
            "      } else {\n"
            "        var duplicate=_EXCLUDE_MOTIF_STATE.items.some(function(item){return String(item.queryText||'')===String(inputText||'');});\n"
            "        if(duplicate){\n"
            "          _setExcludeMotifStatus('That motif is already active.','working');\n"
            "        } else {\n"
            "          _EXCLUDE_MOTIF_STATE.items.push({id:_EXCLUDE_MOTIF_STATE.nextId++,queryText:inputText,displayQuery:originalInput,previewUrl:previewUrl,matchedMolIds:matchedMolIds,removedCount:matchedMolIds.size});\n"
            "          _setExcludeMotifStatus('Added exclusion motif removing '+matchedMolIds.size+' molecules globally. Active motifs: '+_EXCLUDE_MOTIF_STATE.items.length+'.','success');\n"
            "        }\n"
            "      }\n"
            "      _syncExcludeMotifEffects();\n"
            ...
            "  }\n"
            "}\n"
            "async function _runSmilesToSmarts(){\n"
            "  _setSmilesToSmartsStatus('Converting SMILES to SMARTS.','working');\n"
            "  var output=document.getElementById('smiles-to-smarts-output');\n"
            "  try {\n"
            "    var inputText=_getSmilesToSmartsInputText();\n"
            "    if(!inputText){_setSmilesToSmartsStatus('Paste a SMILES string first.','error');if(output){output.value='';}return;}\n"
            "    var RDKit=await _ensureRDKit();\n"
            "    var mol=RDKit.get_mol(inputText);\n"
            "    try {\n"
            "      if(!mol||!mol.is_valid()){throw new Error('Paste a valid SMILES string.');}\n"
            "      var smarts=String(mol.get_smarts()||'').trim();\n"
            "      if(!smarts){throw new Error('Could not generate SMARTS from the provided SMILES.');}\n"
            "      if(output){output.value=smarts;}\n"
            "      _setSmilesToSmartsStatus('SMARTS generated successfully.','success');\n"
            "    } finally { if(mol){mol.delete();} }\n"
            "  } catch (err) {\n"
            "    if(output){output.value='';}\n"
            "    _setSmilesToSmartsStatus((err&&err.message)?err.message:'SMILES to SMARTS conversion failed.','error');\n"
            "  }\n"
            "}\n"
            ...
            "      matchedEntries=_STRUCTURE_SEARCH_ENTRIES.filter(function(entry){return entry.exact_canonical===canonical;});\n"
            ...
            "            if(canonNS){matchedEntries=_STRUCTURE_SEARCH_ENTRIES.filter(function(entry){return entry.exact_canonical_nostereo===canonNS||entry.exact_canonical===canonNS;});}\n"
            "          }\n"
            ...
            "          }\n"
            "        }\n"
            "        var entriesToSearch=_STRUCTURE_SEARCH_ENTRIES;\n"
            "        var hitIndexes=JSON.parse(bundle.library.get_matches(queryMol)||'[]');\n"
            "        matchedEntries=hitIndexes.map(function(idx){return _STRUCTURE_SEARCH_ENTRIES[idx];}).filter(Boolean);\n"
            ...
            "      _STRUCTURE_MATCH_STATE.active=false;\n"
            "      _applyCentralFilters();\n"
            "      _setStructureSearchStatus('No matches found. All scaffolds shown. Try a different query or check if the molecule is in the dataset.','working');\n"
            "    } else {\n"
            ...
            "      _setStructureSearchStatus('Matched '+scaffoldCount+' scaffold(s) and '+memberCount+' member(s). Matching scaffolds are highlighted with a blue border.','success');\n"
            "    }\n"
            "  } catch (err) {\n"
            "    _STRUCTURE_MATCH_STATE.active=false;\n"
            "    _applyCentralFilters();\n"
            ...
            "  var smilesToSmartsRunBtn=document.getElementById('smiles-to-smarts-run');\n"
            "  var smilesToSmartsInput=document.getElementById('smiles-to-smarts-input');\n"
            ...
            "  if(smilesToSmartsRunBtn&&smilesToSmartsRunBtn.dataset.bound!=='1'){smilesToSmartsRunBtn.dataset.bound='1';smilesToSmartsRunBtn.addEventListener('click',_runSmilesToSmarts);}\n"
            "  if(smilesToSmartsInput&&smilesToSmartsInput.dataset.bound!=='1'){smilesToSmartsInput.dataset.bound='1';smilesToSmartsInput.addEventListener('keydown',function(evt){if((evt.ctrlKey||evt.metaKey)&&evt.key==='Enter'){evt.preventDefault();_runSmilesToSmarts();}});}\n"
            "  if(smilesToSmartsInput&&smilesToSmartsInput.dataset.warmupBound!=='1'){smilesToSmartsInput.dataset.warmupBound='1';smilesToSmartsInput.addEventListener('focus',_warmupStructureSearchAssets,{once:true});}\n"
